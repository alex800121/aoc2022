...
monkeyInspects x ms k = ms'
  where
    ...
    ms' =
      V.modify (\v -> MV.write v (fromIntegral k) m') $
...
day11 = do
  ...
  monkeys <- (\x -> V.replicate (length x) undefined V.// x) . fromJust . parseMaybe monkeysParser <$> (getDataDir >>= readFile . (++ "/input/input11.txt"))
